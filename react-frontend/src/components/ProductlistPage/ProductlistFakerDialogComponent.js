...
import productlistFakerFactory from "./productlistFakerFactory";
...
const ProductlistFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = productlistFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("productlist").create(elem));
...
export default connect(null, mapDispatch)(ProductlistFakerDialogComponent);
